...
9. **统计分析**：资产结构分析、负债结构分析、资产负债按到期日趋势（资产/负债以柱状图展示，净资产以曲线展示，悬停柱状图显示当月具体明细）、按到期日汇总表、负债率计算、净资产趋势、分类对比，支持打印全部统计图表
...
| 起始日期 / 结束日期 | 银行存款、理财基金、其他资产（证券除外） | 日期输入框 | 按资产相关日期字段进行日期范围筛选 |

**日期筛选规则：**
- 未输入日期条件时，汇总全部记录
- 起始日期和结束日期都有输入时，汇总起始日期所在月份至结束日期所在月份之间的记录
- 只有结束日期时，汇总结束日期及之前的记录
- 证券类资产不显示日期筛选条件
...
资产页面在筛选区域下方显示汇总卡片，展示当前筛选结果的总笔数和金额合计：

- **总笔数**：当前筛选条件下的记录数量
- **金额合计**：当前筛选条件下各记录金额的总和
- 未输入日期条件时汇总全部匹配记录的金额；输入日期条件时按上述日期规则汇总

### 2.5.2 负债筛选与汇总功能

负债管理页面支持按名称/发卡机构/负债名称、户名、起始日期、结束日期筛选记录：

| 筛选条件 | 适用分类 | 筛选方式 | 说明 |
|----------|----------|----------|------|
| 名称 | 全部负债分类 | 文本模糊匹配（不区分大小写） | 贷款/其他负债按 loanName，信用卡按 institution |
| 户名 | 全部负债分类 | 文本模糊匹配（不区分大小写） | 按 accountName 字段筛选 |
| 起始日期 / 结束日期 | 全部负债分类 | 日期输入框 | 贷款/其他负债按 expectedRepaymentDate，信用卡按 repaymentDate |

**日期筛选与汇总规则**与资产页面一致：未输入日期汇总全部；起止日期都输入时汇总月份区间；只有结束日期时汇总该日及之前。

负债页面同样显示汇总卡片，展示当前筛选结果的总笔数和金额合计。
...
| 资产管理 | 筛选区域 | 按机构名称/银行名称/资产名称、户名、定/活期、起始日期、结束日期筛选当前分类下的资产记录 |
| 资产管理 | 汇总区域 | 显示当前筛选结果的总笔数与金额合计 |
| 资产管理 | 资产列表 | 表格展示已录入资产，支持编辑和删除 |
| 资产管理 | 录入/编辑表单 | 弹窗表单，按分类动态展示字段，自动计算到期日/到期金额/收益 |
| 资产管理 | 打印资产明细 | 按当前分类和筛选条件打印资产明细表 |
| 负债管理 | 分类标签页 | 贷款、信用卡、其他负债三个分类切换 |
| 负债管理 | 筛选区域 | 按名称/发卡机构/负债名称、户名、起始日期、结束日期筛选当前分类下的负债记录 |
| 负债管理 | 汇总区域 | 显示当前筛选结果的总笔数与金额合计 |
| 负债管理 | 负债列表 | 表格展示已录入负债，支持编辑和删除 |
| 负债管理 | 录入/编辑表单 | 弹窗表单，按分类动态展示字段 |
| 负债管理 | 打印负债明细 | 按当前分类和筛选条件打印负债明细表 |
| 统计分析 | 资产结构分析 | 饼图展示各类资产占比 |
| 统计分析 | 负债结构分析 | 饼图展示各类负债占比 |
| 统计分析 | 资产负债对比 | 柱状图对比各分类金额 |
| 统计分析 | 资产负债按到期日趋势 | 组合图：资产/负债以柱状图按月展示，净资产以曲线展示；悬停柱状图显示当月具体资产/负债明细 |
| 统计分析 | 按到期日汇总表 | 表格按资产/负债到期日展示每月资产总额、负债总额、净资产 |
| 统计分析 | 关键指标 | 负债率等 |
| 统计分析 | 打印统计图表 | 一键打印全部统计图表（指标、饼图、柱状图、折线图、汇总表），自动隐藏按钮和导航 |
...
| 资产管理 | 筛选功能 | 按机构名称/银行名称/资产名称、户名、定/活期、起始日期、结束日期筛选后结果正确，重置后恢复；证券类不显示日期筛选 |
| 资产管理 | 汇总功能 | 筛选后正确显示总笔数和金额合计；无日期条件汇总全部，有起止日期汇总月份区间，只有结束日期汇总该日及之前 |
| 资产管理 | 列表排序 | 资产列表默认按到期日升序排序；点击表头可按字段升/降序排序 |
| 资产管理 | 打印资产明细 | 在资产页面可按当前分类和筛选条件打印资产明细表，打印时隐藏原页面内容 |
| 负债管理 | 录入负债 | 贷款、信用卡、其他负债可正常添加、编辑、删除 |
| 负债管理 | 筛选功能 | 按名称/发卡机构/负债名称、户名、起始日期、结束日期筛选后结果正确，重置后恢复 |
| 负债管理 | 汇总功能 | 筛选后正确显示总笔数和金额合计；日期汇总规则与资产页面一致 |
| 负债管理 | 列表排序 | 负债列表默认按还款日升序排序；点击表头可按字段升/降序排序 |
| 负债管理 | 打印负债明细 | 在负债页面可按当前分类和筛选条件打印负债明细表，打印时隐藏原页面内容 |
| 统计分析 | 图表展示 | 资产分布、负债分布、资产负债按到期日趋势等图表正常渲染 |
| 统计分析 | 按到期日趋势 | 资产/负债以柱状图按月展示，净资产以曲线保留；悬停资产柱显示当月资产明细，悬停负债柱显示当月负债明细 |
| 统计分析 | 按到期日汇总表 | 表格按资产/负债到期日展示每月资产总额、负债总额、净资产，无数据时显示提示 |
| 设置 | 数据库位置 | 可切换数据库文件存放位置，切换后自动加载新位置的 db 文件 |
| 设置 | 恢复默认 | 可一键恢复默认数据库位置，恢复后加载默认位置的 db 文件 |
| 统计分析 | 打印统计图表 | 点击"打印统计图表"按钮可打印全部统计内容（指标、饼图、柱状图、折线图、汇总表），打印时隐藏导航和按钮 |

### 5.2 数据验收

| 验收项 | 通过标准 |
|--------|----------|
| 数据持久化 | 重启应用后资产/负债/变动记录不丢失；应用关闭前确保 debounced 写入已 flush 到磁盘 |
| SQLite 存储 | 数据保存到本地 SQLite 文件（默认 `%APPDATA%/WealthCare/WealthCare.db`） |
| 配置独立 | 数据库路径配置保存在 `%APPDATA%/WealthCare/config.json`，与 db 文件分离 |
| 旧数据库迁移 | 首次启动检测到历史版本数据库（包括 `wealth-tracker.db` 及各历史路径）时，自动迁移数据到新的 `WealthCare.db`；迁移后仅使用 `WealthCare.db` |
| Schema 自动修复 | 当 SQLite 表缺少必要列或表结构不一致时，自动重建表并恢复数据，保证新旧版本兼容 |
| 去除旧引用 | 应用 UI 与配置中不再出现 `wealth-tracker.db` 相关引用 |
